...
let latLng (address:string) =
    let fetch = sprintf @"http://maps.googleapis.com/maps/api/geocode/json?region=uk&address=%s&sensor=false" (address.Replace(" ","+") + "+Whitstable")
...
let item1 =
    { Name = "Harbour Cottage"
      ItemCategory = Cottage
      Address = "20 Woodlawn St"
      Description = "hello"
      LatLng = Some(0M,0M)
      Colour = "Red"}
...
let file = System.IO.File.OpenText(projectPath + "Whitstable Details.csv").ReadToEnd().Trim().Replace(dodgyChar,' ').Replace("'","")

let itemsArray = file.Split([|'\n'|]) |> Array.map (fun i -> let arr = i.Trim().Split([|','|])

                                                             printfn "Processing %s , %s" arr.[0] arr.[2]
                                                             let cat = getCategory arr.[2]
...
let outputFile = System.IO.File.CreateText(projectPath + "whitstableTest.js")
